...
Files already referenced by Rekordbox are never overwritten blindly. The converter stages a new output and reuses the referenced target only when their decoded-audio SHA-256 hashes match. Unreferenced targets require separate replacement confirmation and are added to rollback and full restoration.
